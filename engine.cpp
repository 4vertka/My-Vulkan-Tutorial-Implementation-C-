...
void HelloTriangle::createInstance() {
  ...
  if (enableValidationLayers && !checkValidationLayerSupport()) {
    throw std::runtime_error("validation layers requested, nut not available");
  }
...
void HelloTriangle::pickPhysicalDevice() {
  ...
  if (deviceCount == 0) {
    throw std::runtime_error("failed to find GPU width vulkan support");
  }
...
void HelloTriangle::recordCommandBuffer(VkCommandBuffer commandBuffer,
                                        uint32_t imageIndex) {
...
void HelloTriangle::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer,
                               VkDeviceSize size) {
...
void HelloTriangle::createTextureImage() {
  ...
  // stbi_uc* pixels = stbi_load("./textures/saul.jpg", &texWidth, &texHeight,
...
void HelloTriangle::createImage(uint32_t width, uint32_t height,
                                uint32_t mipLevels,
...
void HelloTriangle::initCamera() {
  ...
  inputHandler = std::make_unique<InputHandler>(window, camera.get());
}
